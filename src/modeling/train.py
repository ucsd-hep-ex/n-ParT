...
def normalize_matrices(model):
    ...
    try:
        ...
        model.encoder.input_proj.weight.data = model.encoder.input_proj.weight.data.to(
            dtype=torch.float32
        )
        ...
        for block in model.encoder.blocks:
            # QKV projection
            block.qkv.weight.data = block.qkv.weight.data.to(dtype=torch.float32)
            block.qkv.weight.data.copy_(ModelUtils.justnorm(block.qkv.weight.data, 1)).to(
                dtype=torch.bfloat16
            )

            # Attention output projection
            block.att_c_proj.weight.data = block.att_c_proj.weight.data.to(dtype=torch.float32)
            block.att_c_proj.weight.data.copy_(
                ModelUtils.justnorm(block.att_c_proj.weight.data, 0)
            ).to(dtype=torch.bfloat16)

            # MLP layers
            block.c_fc.weight.data = block.c_fc.weight.data.to(dtype=torch.float32)
            block.c_fc.weight.data.copy_(ModelUtils.justnorm(block.c_fc.weight.data, 1)).to(
                dtype=torch.bfloat16
            )

            block.mlp_c_proj.weight.data = block.mlp_c_proj.weight.data.to(dtype=torch.float32)
            block.mlp_c_proj.weight.data.copy_(
                ModelUtils.justnorm(block.mlp_c_proj.weight.data, 0)
            ).to(dtype=torch.bfloat16)

        # Normalize projector layers
        for layer in model.projector.layers:
            # UV gating linear layer
            layer["linear"].weight.data = layer["linear"].weight.data.to(dtype=torch.float32)
            layer["linear"].weight.data.copy_(
                ModelUtils.justnorm(layer["linear"].weight.data, 1)
            ).to(dtype=torch.bfloat16)

            # Projection layer
            layer["proj"].weight.data = layer["proj"].weight.data.to(dtype=torch.float32)
            layer["proj"].weight.data.copy_(ModelUtils.justnorm(layer["proj"].weight.data, 0)).to(
                dtype=torch.bfloat16
            )
...
def main(args):
    ...
    for epoch in range(epoch_start, args.n_epochs):
        ...
        pbar = tqdm(data_iter, total=len(train_dataloader), desc="Training")
